...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Hive failed to launch";
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Hive failed to launch")
                        .body(Some(
                            format!(
...
fn main() {
    ...
    // args are appended verbatim and would otherwise be misinterpreted as Hive's
    // own arguments.
    ...
    if args.system_specs {
        ...
        println!("Hive System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    app.run(move |cx| {
        ...
        // hive: stripped telemetry.start(...) — no dial-home to zed.dev
        ...
        // hive: stripped copilot_chat::init(...) and copilot_ui::init(...)
        ...
        // hive: stripped edit_prediction_ui::init(cx)
        web_search::init(cx);
        web_search_providers::init(app_state.client.clone(), app_state.user_store.clone(), cx);
        snippet_provider::init(cx);
        // hive: stripped edit_prediction_registry::init(...)
        ...
        // hive: stripped edit_prediction::init(cx)
...
pub(crate) async fn restore_or_create_workspace(
    ...
    if let Some(multi_workspaces) = restorable_workspaces(cx, &app_state).await {
        ...
        // check, Hive would exit silently.
                ...
                workspace::open_new(
                    ...
                    |workspace, window, cx| {
                        ...
                        match restore_on_startup {
                            ...
                            _ => {
                                crate::zed::open_terminal_or_fallback_editor(
                                    workspace, window, cx,
                                );
                            }
                        }
            ...
            workspace::open_new(
                ...
                |workspace, window, cx| {
                    ...
                    match restore_on_startup {
                        workspace::RestoreOnStartupBehavior::Launchpad => {}
                        _ => {
                            crate::zed::open_terminal_or_fallback_editor(workspace, window, cx);
                        }
                    }
                },
            )
...
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/Hive`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zed`.
    /// On Windows, the default is `%LOCALAPPDATA%\Hive`.
    ...
    /// prevents Hive from starting, so you can't run `zed: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having Hive run a separate
    ...
    /// The PID of the Hive process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    ...
    /// Unix socket path for IPC with the parent Hive process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
